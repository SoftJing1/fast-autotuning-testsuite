; ModuleID = 'kernels/kernel-template/gaussian_static_1.cl'
source_filename = "kernels/kernel-template/gaussian_static_1.cl"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

; Function Attrs: convergent nofree norecurse nounwind memory(argmem: readwrite) uwtable
define spir_kernel void @gaussian_1(ptr noalias nocapture noundef readonly align 4 %0, ptr noalias nocapture noundef readnone align 4 %1, ptr noalias nocapture noundef writeonly align 4 %2) local_unnamed_addr #0 !kernel_arg_addr_space !6 !kernel_arg_access_qual !7 !kernel_arg_type !8 !kernel_arg_base_type !8 !kernel_arg_type_qual !9 {
  %4 = tail call i64 @_Z12get_group_idj(i32 noundef 0) #3
  %5 = shl i64 %4, 9
  %6 = getelementptr i8, ptr %0, i64 4112
  %7 = getelementptr i8, ptr %0, i64 8224
  %8 = getelementptr i8, ptr %0, i64 12336
  %9 = getelementptr i8, ptr %0, i64 16448
  br label %10

10:                                               ; preds = %3, %21
  %11 = phi i64 [ 0, %3 ], [ %22, %21 ]
  %12 = mul nuw nsw i64 %11, 1028
  %13 = getelementptr float, ptr %0, i64 %12
  %14 = getelementptr float, ptr %6, i64 %12
  %15 = getelementptr float, ptr %7, i64 %12
  %16 = getelementptr float, ptr %8, i64 %12
  %17 = getelementptr float, ptr %9, i64 %12
  %18 = shl nuw nsw i64 %11, 12
  %19 = getelementptr i8, ptr %2, i64 %18
  br label %24

20:                                               ; preds = %21
  ret void

21:                                               ; preds = %24
  %22 = add nuw nsw i64 %11, 1
  %23 = icmp eq i64 %22, 1024
  br i1 %23, label %20, label %10

24:                                               ; preds = %10, %24
  %25 = phi i64 [ 0, %10 ], [ %107, %24 ]
  %26 = add nuw nsw i64 %25, %5
  %27 = getelementptr float, ptr %13, i64 %26
  %28 = load float, ptr %27, align 4, !tbaa !10
  %29 = add i64 %26, 1
  %30 = getelementptr float, ptr %13, i64 %29
  %31 = load float, ptr %30, align 4, !tbaa !10
  %32 = fmul float %31, 4.000000e+00
  %33 = tail call float @llvm.fmuladd.f32(float %28, float 2.000000e+00, float %32)
  %34 = add i64 %26, 2
  %35 = getelementptr float, ptr %13, i64 %34
  %36 = load float, ptr %35, align 4, !tbaa !10
  %37 = tail call float @llvm.fmuladd.f32(float %36, float 5.000000e+00, float %33)
  %38 = add i64 %26, 3
  %39 = getelementptr float, ptr %13, i64 %38
  %40 = load float, ptr %39, align 4, !tbaa !10
  %41 = tail call float @llvm.fmuladd.f32(float %40, float 4.000000e+00, float %37)
  %42 = add i64 %26, 4
  %43 = getelementptr float, ptr %13, i64 %42
  %44 = load float, ptr %43, align 4, !tbaa !10
  %45 = tail call float @llvm.fmuladd.f32(float %44, float 2.000000e+00, float %41)
  %46 = getelementptr float, ptr %14, i64 %26
  %47 = load float, ptr %46, align 4, !tbaa !10
  %48 = tail call float @llvm.fmuladd.f32(float %47, float 4.000000e+00, float %45)
  %49 = getelementptr float, ptr %14, i64 %29
  %50 = load float, ptr %49, align 4, !tbaa !10
  %51 = tail call float @llvm.fmuladd.f32(float %50, float 9.000000e+00, float %48)
  %52 = getelementptr float, ptr %14, i64 %34
  %53 = load float, ptr %52, align 4, !tbaa !10
  %54 = tail call float @llvm.fmuladd.f32(float %53, float 1.200000e+01, float %51)
  %55 = getelementptr float, ptr %14, i64 %38
  %56 = load float, ptr %55, align 4, !tbaa !10
  %57 = tail call float @llvm.fmuladd.f32(float %56, float 9.000000e+00, float %54)
  %58 = getelementptr float, ptr %14, i64 %42
  %59 = load float, ptr %58, align 4, !tbaa !10
  %60 = tail call float @llvm.fmuladd.f32(float %59, float 4.000000e+00, float %57)
  %61 = getelementptr float, ptr %15, i64 %26
  %62 = load float, ptr %61, align 4, !tbaa !10
  %63 = tail call float @llvm.fmuladd.f32(float %62, float 5.000000e+00, float %60)
  %64 = getelementptr float, ptr %15, i64 %29
  %65 = load float, ptr %64, align 4, !tbaa !10
  %66 = tail call float @llvm.fmuladd.f32(float %65, float 1.200000e+01, float %63)
  %67 = getelementptr float, ptr %15, i64 %34
  %68 = load float, ptr %67, align 4, !tbaa !10
  %69 = tail call float @llvm.fmuladd.f32(float %68, float 1.500000e+01, float %66)
  %70 = getelementptr float, ptr %15, i64 %38
  %71 = load float, ptr %70, align 4, !tbaa !10
  %72 = tail call float @llvm.fmuladd.f32(float %71, float 1.200000e+01, float %69)
  %73 = getelementptr float, ptr %15, i64 %42
  %74 = load float, ptr %73, align 4, !tbaa !10
  %75 = tail call float @llvm.fmuladd.f32(float %74, float 5.000000e+00, float %72)
  %76 = getelementptr float, ptr %16, i64 %26
  %77 = load float, ptr %76, align 4, !tbaa !10
  %78 = tail call float @llvm.fmuladd.f32(float %77, float 4.000000e+00, float %75)
  %79 = getelementptr float, ptr %16, i64 %29
  %80 = load float, ptr %79, align 4, !tbaa !10
  %81 = tail call float @llvm.fmuladd.f32(float %80, float 9.000000e+00, float %78)
  %82 = getelementptr float, ptr %16, i64 %34
  %83 = load float, ptr %82, align 4, !tbaa !10
  %84 = tail call float @llvm.fmuladd.f32(float %83, float 1.200000e+01, float %81)
  %85 = getelementptr float, ptr %16, i64 %38
  %86 = load float, ptr %85, align 4, !tbaa !10
  %87 = tail call float @llvm.fmuladd.f32(float %86, float 9.000000e+00, float %84)
  %88 = getelementptr float, ptr %16, i64 %42
  %89 = load float, ptr %88, align 4, !tbaa !10
  %90 = tail call float @llvm.fmuladd.f32(float %89, float 4.000000e+00, float %87)
  %91 = getelementptr float, ptr %17, i64 %26
  %92 = load float, ptr %91, align 4, !tbaa !10
  %93 = tail call float @llvm.fmuladd.f32(float %92, float 2.000000e+00, float %90)
  %94 = getelementptr float, ptr %17, i64 %29
  %95 = load float, ptr %94, align 4, !tbaa !10
  %96 = tail call float @llvm.fmuladd.f32(float %95, float 4.000000e+00, float %93)
  %97 = getelementptr float, ptr %17, i64 %34
  %98 = load float, ptr %97, align 4, !tbaa !10
  %99 = tail call float @llvm.fmuladd.f32(float %98, float 5.000000e+00, float %96)
  %100 = getelementptr float, ptr %17, i64 %38
  %101 = load float, ptr %100, align 4, !tbaa !10
  %102 = tail call float @llvm.fmuladd.f32(float %101, float 4.000000e+00, float %99)
  %103 = getelementptr float, ptr %17, i64 %42
  %104 = load float, ptr %103, align 4, !tbaa !10
  %105 = tail call float @llvm.fmuladd.f32(float %104, float 2.000000e+00, float %102)
  %106 = getelementptr float, ptr %19, i64 %26
  store float %105, ptr %106, align 4, !tbaa !10
  %107 = add nuw nsw i64 %25, 1
  %108 = icmp eq i64 %107, 512
  br i1 %108, label %21, label %24
}

; Function Attrs: convergent mustprogress nofree nounwind willreturn memory(none)
declare i64 @_Z12get_group_idj(i32 noundef) local_unnamed_addr #1

; Function Attrs: mustprogress nocallback nofree nosync nounwind speculatable willreturn memory(none)
declare float @llvm.fmuladd.f32(float, float, float) #2

attributes #0 = { convergent nofree norecurse nounwind memory(argmem: readwrite) uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" "uniform-work-group-size"="false" }
attributes #1 = { convergent mustprogress nofree nounwind willreturn memory(none) "frame-pointer"="all" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #2 = { mustprogress nocallback nofree nosync nounwind speculatable willreturn memory(none) }
attributes #3 = { convergent nounwind willreturn memory(none) }

!llvm.module.flags = !{!0, !1, !2, !3}
!opencl.ocl.version = !{!4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"uwtable", i32 2}
!3 = !{i32 7, !"frame-pointer", i32 2}
!4 = !{i32 2, i32 0}
!5 = !{!"clang version 20.1.0"}
!6 = !{i32 1, i32 1, i32 1}
!7 = !{!"none", !"none", !"none"}
!8 = !{!"float*", !"float*", !"float*"}
!9 = !{!"restrict const", !"restrict", !"restrict"}
!10 = !{!11, !11, i64 0}
!11 = !{!"float", !12, i64 0}
!12 = !{!"omnipotent char", !13, i64 0}
!13 = !{!"Simple C/C++ TBAA"}
