"""Generate tuned versions of GEMM kernels using OpenTuner.
...
Usage:
    python -m scripts.generate_gemm --help
    python -m scripts.generate_gemm list-params
    python -m scripts.generate_gemm generate --config config.json --output kernel.cl
    python -m scripts.generate_gemm exhaustive --input-size 256 256 256 --output-dir ./kernels/
"""
...
def get_parameters_definition(M: int, N: int, K: int) -> ConfigurationManipulator:
    """Define the OpenTuner parameter search space for GEMM kernel tuning.
    
    This function encapsulates all tuning parameters and their constraints
    based on the OpenTuner example in gemm.cpp.
    ...
    # OCL dimensions - 3D work space
    manipulator.add_parameter(IntegerParameter('OCL_DIM_L_1', 0, 2))
    manipulator.add_parameter(IntegerParameter('OCL_DIM_L_2', 0, 2))
    manipulator.add_parameter(IntegerParameter('OCL_DIM_R_1', 0, 2))
    ...
    # L_2 (N) dimension parameters
    manipulator.add_parameter(IntegerParameter('INPUT_SIZE_L_2', N, N))
    manipulator.add_parameter(IntegerParameter('L_CB_SIZE_L_2', 1, N))
    manipulator.add_parameter(IntegerParameter('P_CB_SIZE_L_2', 1, N))
    manipulator.add_parameter(IntegerParameter('NUM_WG_L_2', 1, N))
    manipulator.add_parameter(IntegerParameter('NUM_WI_L_2', 1, N))

    # R_1 (K) dimension parameters
    manipulator.add_parameter(IntegerParameter('INPUT_SIZE_R_1', K, K))
    manipulator.add_parameter(IntegerParameter('L_CB_SIZE_R_1', 1, K))
    manipulator.add_parameter(IntegerParameter('P_CB_SIZE_R_1', 1, K))
    manipulator.add_parameter(IntegerParameter('NUM_WG_R_1', 1, K))
    manipulator.add_parameter(IntegerParameter('NUM_WI_R_1', 1, K))
    ...
    manipulator.add_parameter(IntegerParameter('L_WRITE_BACK', 2, 2))
...
def get_parameters_info() -> Dict:
    ...
    return {
        ...
        "ocl_dimensions": {
            "OCL_DIM_L_1": "OpenCL dimension for L_1 (M) in {0, 1, 2}",
            "OCL_DIM_L_2": "OpenCL dimension for L_2 (N) in {0, 1, 2}",
            "OCL_DIM_R_1": "OpenCL dimension for R_1 (K) in {0, 1, 2}",
        },
        "dimension_l1_parameters": [
            "INPUT_SIZE_L_1", "L_CB_SIZE_L_1", "P_CB_SIZE_L_1", "NUM_WG_L_1", "NUM_WI_L_1"
        ],
        "dimension_l2_parameters": [
...
def is_configuration_valid(
    ...
    Constraints are based on the validation logic from gemm.cpp:
    ...
    if cfg['NUM_WI_L_1'] > max_wi_size[cfg['OCL_DIM_L_1']]:
        return False
    if cfg['NUM_WI_L_2'] > max_wi_size[cfg['OCL_DIM_L_2']]:
        return False
    if cfg['NUM_WI_R_1'] > max_wi_size[cfg['OCL_DIM_R_1']]:
        return False
    if cfg['NUM_WI_L_1'] * cfg['NUM_WI_L_2'] * cfg['NUM_WI_R_1'] > max_wg_size:
        return False

    # Dimension constraints - all three must be different
    dims = {cfg['OCL_DIM_L_1'], cfg['OCL_DIM_L_2'], cfg['OCL_DIM_R_1']}
    if len(dims) != 3:
        return False
    ...
    # L_2 divisibility constraints
    if cfg['INPUT_SIZE_L_2'] % cfg['L_CB_SIZE_L_2'] != 0:
        return False
    rem = cfg['INPUT_SIZE_L_2'] // cfg['L_CB_SIZE_L_2']
    if rem % cfg['NUM_WG_L_2'] != 0:
        return False
    rem = rem // cfg['NUM_WG_L_2']
    if rem % cfg['NUM_WI_L_2'] != 0:
        return False
    rem = rem // cfg['NUM_WI_L_2']
    if rem != cfg['P_CB_SIZE_L_2']:
        return False
...
def generate_tuned_kernel(
    ...
    Args:
        ...
        template_path: Path to kernel template (defaults to gemm_1.cl)
...
def exhaustive_iterate_configurations(
    ...
    # OCL dimension choices - must be all different
    ocl_dim_choices = [
    ...
    valid_l2 = get_valid_dimension_factors(N, max_wi_size[1], max_wg_size)
    valid_r1 = get_valid_dimension_factors(K, max_wi_size[2], max_wg_size)
        ...
        for cb_res_combo in product(*cb_res_values):
            ...
            for ocl_dims in ocl_dim_choices:
                ocl_dict = {
                    ...
                    'OCL_DIM_L_2': ocl_dims[1],
                    'OCL_DIM_R_1': ocl_dims[2],
                }
                ...
                for l1_idx in range(len(valid_l1['L_CB_SIZE'])):
                    ...
                    # Build valid L_2 tuples
                    for l2_idx in range(len(valid_l2['L_CB_SIZE'])):
                        l2_dict = {
                            'L_CB_SIZE_L_2': valid_l2['L_CB_SIZE'][l2_idx],
                            'NUM_WG_L_2': valid_l2['NUM_WG'][l2_idx],
                            'NUM_WI_L_2': valid_l2['NUM_WI'][l2_idx],
                            'P_CB_SIZE_L_2': valid_l2['P_CB_SIZE'][l2_idx],
                        }

                        # Build valid R_1 tuples with cross-dimension constraint
                        wi_product = l1_dict['NUM_WI_L_1'] * l2_dict['NUM_WI_L_2']
                        valid_r1_filtered = get_valid_dimension_factors(
                            K, max_wi_size[2], max_wg_size, wi_product
                        )
...
def count_valid_configurations(
    ...
    """
    count = 0
    for _ in exhaustive_iterate_configurations(M, N, K, max_wi_size, max_wg_size):
        count += 1
    return count
...
def random_sample_configurations(
    ...
    num_l2_options = len(valid_l2['L_CB_SIZE'])

    if num_l1_options == 0 or num_l2_options == 0:
        print(f"Warning: No valid dimension configurations for M={M}, N={N}", file=sys.stderr)
        return
    ...
    while valid < num_samples:
        ...
        l2_idx = random.randint(0, num_l2_options - 1)
        l2_dict = {
            'L_CB_SIZE_L_2': valid_l2['L_CB_SIZE'][l2_idx],
            'NUM_WG_L_2': valid_l2['NUM_WG'][l2_idx],
            'NUM_WI_L_2': valid_l2['NUM_WI'][l2_idx],
...
def cmd_list_params_for_size(args) -> int:
    ...
    M, N, K = args.input_size[0], args.input_size[1], args.input_size[2]
    manipulator = get_parameters_definition(M, N, K)
...
def cmd_exhaustive(args) -> int:
    ...
    if args.count_only:
        ...
        count = count_valid_configurations(M, N, K, max_wi_size, max_wg_size)
    ...
    if args.random:
        ...
        config_iter = random_sample_configurations(
            M, N, K, num_samples, max_wi_size, max_wg_size, seed, attempts_per_sample
        )
        if args.verbose:
            print(
        ...
        config_iter = exhaustive_iterate_configurations(M, N, K, max_wi_size, max_wg_size)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate tuned GEMM kernels with different parameter configurations"
    )
    ...
    parser_size.add_argument('input_size', type=int, nargs=3, metavar=('M', 'N', 'K'),
                             help='Input dimensions (M N K)')
    ...
    parser_exh.add_argument('input_size', type=int, nargs=3, metavar=('M', 'N', 'K'),
                            help='Input dimensions (M N K)')
