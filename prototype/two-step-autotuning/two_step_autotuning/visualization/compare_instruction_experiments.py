...
def plot_case_compare(
	...
	for label, run_root in experiments:
		aggregates[label] = _aggregate_method(run_root / case_dir_name, "instruction_map_tuning")
...
def plot_seed_compare(
	...
	for index, (label, run_root) in enumerate(experiments):
		trace_path = run_root / case_dir_name / f"seed_{seed}" / "instruction_map_tuning" / "trace.csv"
...
def build_report(
	...
	for case_dir_name, title in CASES:
		plot_path = output_dir / case_dir_name / "instruction_map_compare.png"
		...
		for seed in sorted(case_seeds):
			seed_plot_path = output_dir / case_dir_name / f"seed_{seed}" / "instruction_map_compare.png"
			plot_seed_compare(
				case_dir_name,
				seed,
				f"{title} - Seed {seed}",
			...
			seed_figure_blocks.append(
				...
				<div class="seed-figure">
					<h3>Seed {seed}</h3>
					<img src="{html.escape(str(seed_plot_rel))}" alt="{html.escape(title)} seed {seed} instruction-map experiment comparison">
				</div>
				"""
			)
		...
		sections.append(
			...
			<section>
				...
				<img src="{html.escape(str(plot_rel))}" alt="{html.escape(title)} instruction-map experiment comparison">
				<p class="caption">Each line is the median instruction-map tuning convergence over the recorded seeds. The shaded band shows the interquartile range.</p>
				<div class="seed-grid">
...
<head>
	...
	<title>Instruction-Map Experiment Comparison</title>
	<style>
...
<body>
	<header>
		<h1>Instruction-Map Convergence Comparison</h1>
...
def main() -> int:
	parser = argparse.ArgumentParser(description="Compare instruction-map tuning convergence across experiment roots.")
	parser.add_argument(
		"--experiment",
		dest="experiments",
		action="append",
