...
from ..core.parameter_distance import (
	PARAMETER_DISTANCE_METRICS,
	ParameterDistanceModel,
	neighbor_results_to_dicts,
)
...
def _summarize(case: tuple[str, str], metric: str, rows: list[dict]) -> dict:
...
def evaluate_case(
	...
	model = ParameterDistanceModel(dataset, specs)
	...
	for metric in metrics:
		results = model.nearest_neighbors(metric, base_indices)
		rows = neighbor_results_to_dicts(results)
		for row in rows:
			row["kernel"] = case[0]
			row["input_size"] = case[1]
		detail_rows.extend(rows)
		summary_rows.append(_summarize(case, metric, rows))
	return detail_rows, summary_rows


def build_argparser() -> argparse.ArgumentParser:
	parser = argparse.ArgumentParser(
		description="Evaluate parameter-space distance metrics by nearest-neighbor runtime similarity."
	)
	...
	parser.add_argument(
		...
		action="append",
		default=[],
		help="Metric to evaluate. May be repeated. Defaults to all metrics.",
	)
	parser.add_argument("--sample-count", type=int, default=None)
	parser.add_argument("--random-seed", type=int, default=1)
	parser.add_argument(
		"--output-dir",
		default="prototype/two-step-autotuning/runs/parameter_distance_metrics",
	)
	return parser
...
def main() -> int:
	...
	metrics = tuple(args.metric) if args.metric else PARAMETER_DISTANCE_METRICS
	...
	all_summaries = []
	for case in cases:
		details, summaries = evaluate_case(
			db_path=db_path,
			case=case,
			metrics=metrics,
	...
	for row in sorted(
		all_summaries,
		key=lambda item: (item["kernel"], item["input_size"], item["median_relative_runtime_diff_pct"]),
	):
		print(
			f"{row['kernel']} {row['input_size']} {row['metric']}: "
			...
			f"mean ties {row['mean_tie_count']:.1f}"
		)
	return 0
